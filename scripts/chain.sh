#!/usr/bin/env bash
# Start anvil + FHEVM cleartext host stack + CipherGift (vault + packet
# contract) in one command.
#
# Flow (2 terminals):
#   pnpm chain   # this script — anvil + FHEVM host + CipherGift
#   pnpm start   # frontend
#
# To redeploy CipherGift without restarting anvil, run
# `pnpm deploy:localhost` in another terminal.
#
# Troubleshooting:
#   "max fee per gas less than block base fee" / "deploy_raw_contract
#   returned no contractAddress" — anvil's restored state has a base fee
#   spike. Delete .anvil-state.json and re-run pnpm chain.
set -euo pipefail
...
echo "deploying CipherGift..."
RPC_URL="$RPC_URL" "$SCRIPT_DIR/deploy-localhost.sh"

echo
echo "✓ anvil + FHEVM host + CipherGift ready on $RPC_URL (chain id 31337)"
