#!/usr/bin/env bash
# Deploy CipherGift (vault + packet contract) to a running anvil node at
# 127.0.0.1:8545 and regenerate the frontend's per-contract ABI/address
# files.
#
...
if ! PRIVATE_KEY="$ANVIL_PK" forge script script/DeployCipherGift.s.sol:DeployCipherGift \
    --rpc-url "$RPC_URL" \
...
grep -E "ConfidentialETHVault|CipherGift|Owner|Orchestrator|===" "$deploy_log" || true
...
echo "    packages/site/contracts/*.ts (+ *.local.ts)."
